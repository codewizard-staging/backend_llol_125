package com.app.llol.integrationtest;
...
class ControllerTest {
  ...
  void  testRetrieveMetadataDocument() {
    ...
    assertNotNull(n);
    assertEquals("llol", n.getAttribute("Namespace"));
  ...
  void  testCreateEmployeePersonalInfoInstance() throws IOException {
    given()
        ...
        .when()
        .post("/llol/EmployeePersonalInfoes")
        .then()
        .statusCode(HttpStatusCode.CREATED.getStatusCode());
  ...
  given()
   ...
   given()
            .when()
            .get("/llol/EmployeePersonalInfoes?$top=1")
            .then()
            .statusCode(HttpStatusCode.fromStatusCode(200).getStatusCode())
    ...
    given()
            .when()
            .get("/llol/EmployeePersonalInfoes/$count")
            .then()
            .statusCode(HttpStatusCode.fromStatusCode(200).getStatusCode())
            .body(is("1"));
  ...
  given()
   ...
   given()
            .when()
            .get("/llol/EmlpoyeeProfiles?$top=1")
            .then()
            .statusCode(HttpStatusCode.fromStatusCode(200).getStatusCode())
    ...
    given()
            .when()
            .get("/llol/EmlpoyeeProfiles/$count")
            .then()
            .statusCode(HttpStatusCode.fromStatusCode(200).getStatusCode())
            .body(is("1"));
  ...
  @AfterEach
  void  teardown() {
    jdbcTemplate.execute("DELETE FROM llol_772.Organization");
    jdbcTemplate.execute("DELETE FROM llol_772.EmployeePersonalInfo");
    jdbcTemplate.execute("DELETE FROM llol_772.EmlpoyeeProfile");
